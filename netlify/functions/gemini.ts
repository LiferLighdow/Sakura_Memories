// Important: This file must be placed in the `/netlify/functions` directory.
import type { Handler, HandlerEvent } from "@netlify/functions";
import { GoogleGenAI, Type } from "@google/genai";
import type { Affection, Character } from '../../types';

// This code runs on the server. The API_KEY is securely accessed from Netlify's environment variables.
if (!process.env.API_KEY) {
    throw new Error("API_KEY environment variable not set in Netlify project settings");
}
...
export const handler: Handler = async (event: HandlerEvent) => {
    ...
    try {
        ...
        console.error(`Error in Netlify function for action:`, error);
